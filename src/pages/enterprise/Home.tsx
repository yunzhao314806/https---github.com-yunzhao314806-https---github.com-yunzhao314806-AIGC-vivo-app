  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-8 py-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h1 className="text-xl font-bold text-foreground text-balance">企业招聘中心</h1>
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogTrigger asChild>
            <Button size="sm"><Plus className="w-4 h-4 mr-2" />发布职位</Button>
          </DialogTrigger>
          <DialogContent className="max-w-[calc(100%-2rem)] md:max-w-lg max-h-[90dvh] overflow-y-auto">
            ...
            <div className="space-y-4 pt-2">
              <div className="space-y-2">
                ...
                <Input value={form.title} onChange={e => updateForm('title', e.target.value)} placeholder="例如：前端开发工程师" className="px-3" />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input value={form.location} onChange={e => updateForm('location', e.target.value)} placeholder="例如：北京市朝阳区" className="px-3" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-2">
                  <Label className="text-sm font-normal">最低薪资(元/月)</Label>
                  <Input type="number" value={form.salary_min} onChange={e => updateForm('salary_min', e.target.value)} placeholder="15000" className="px-3" />
                </div>
                <div className="space-y-2">
                  <Label className="text-sm font-normal">最高薪资(元/月)</Label>
                  <Input type="number" value={form.salary_max} onChange={e => updateForm('salary_max', e.target.value)} placeholder="25000" className="px-3" />
                </div>
              </div>
              <div className="space-y-2">
                <Label className="text-sm font-normal">技能要求(逗号分隔)</Label>
                <Input value={form.skills_required} onChange={e => updateForm('skills_required', e.target.value)} placeholder="React, TypeScript, Node.js" className="px-3" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-2">
                  <Label className="text-sm font-normal">工作经验</Label>
                  <Input value={form.experience_required} onChange={e => updateForm('experience_required', e.target.value)} placeholder="3年以上" className="px-3" />
                </div>
                <div className="space-y-2">
                  <Label className="text-sm font-normal">学历要求</Label>
                  <Input value={form.education_required} onChange={e => updateForm('education_required', e.target.value)} placeholder="本科及以上" className="px-3" />
                </div>
              </div>
              <div className="space-y-2">
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base">我的职位</CardTitle>
            <Button variant="ghost" size="sm" asChild>
              <Link to="/enterprise/recruitment">全部 <ChevronRight className="w-4 h-4 ml-1" /></Link>
            </Button>
          </div>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="py-8 text-center text-muted-foreground">
              ...
              <p>还没有发布职位，点击"发布职位"开始招聘</p>
            </div>
          ) : (
            <div className="space-y-3">
              {jobs.map(job => (
                <div key={job.id} className="flex items-center gap-3 p-3 rounded-md border border-border hover:bg-muted/30 transition-colors">
